...
// Keepalive ping — wakes Render server before user needs it
function keepalive() {
  fetchWithTimeout('/api/products', 30000).catch(() => {});
}
// Ping immediately so server wakes on page load
keepalive();
...
async function fetchProducts(retryCount = 0) {
  ...
  try {
    const res = await fetchWithTimeout('/api/products', 12000);
    ...
    if (!PRODUCTS.length) {
      // Auto-retry once after 5s (server may be cold-starting)
      if (retryCount === 0) {
        showRetryBanner(true); // show "warming up" state
        setTimeout(() => fetchProducts(1), 5000);
      } else {
        // Second failure — show manual retry
...
function showRetryBanner(isAutoRetry) {
  const grids = ['ourBooksGrid', 'featuredGrid'];
  grids.forEach((id, i) => {
    const el = document.getElementById(id);
    if (!el) return;
    if (i > 0 && isAutoRetry) return; // only show one banner while auto-retrying
    el.innerHTML = `
      <div class="books-retry-banner">
        <p>${isAutoRetry
          ? '⏳ Server is warming up… retrying in 5s'
          : '⚠️ Could not load books. Please try again.'}</p>
        ${!isAutoRetry ? `<button onclick="fetchProducts()">Retry</button>` : ''}
      </div>`;
  });
}
