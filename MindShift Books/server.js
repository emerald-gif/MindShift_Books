...
app.post('/api/pay', async (req, res) => {
  try {
    const { email, productId, productIds } = req.body;
    ...
    const metadata = { productIds: ids, items, usd_total: usdTotal, fx_rate: fxRate, ngn_charged: ngnAmount };
  ...
  try {
    ...
    if (BREVO_API_KEY && userEmail && items.length) {
      try {
        ...
        // gets lost for multi-book orders. {{ params.items_html }} is also sent
        // in case you want to render a proper bullet list in the template instead.
        const downloadLinks = items.length === 1
          ? items[0].pdfUrl
          : items.map(it => `${it.title}: ${it.pdfUrl}`).join('\n');
        const itemsHtml = items.map(it => `<li><a href="${it.pdfUrl}">${it.title}</a></li>`).join('');
