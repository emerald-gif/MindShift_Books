...
// ── Permanent per-order download tokens ────────────────────────────────────
// Tokens are generated once at purchase, stored in Firestore alongside the
// order, and never expire. They're cryptographically random (unguessable)
// so they protect against casual piracy without punishing legitimate buyers
// who come back days or weeks later to re-download.
// DOWNLOAD_SECRET adds an extra layer — even if someone gets the token string,
// the server also verifies it was signed with the secret, so a token that
// wasn't generated by this server is rejected.
const DOWNLOAD_SECRET = process.env.DOWNLOAD_SECRET || crypto.randomBytes(32).toString('hex');

function mintDownloadToken(orderId, productId) {
  const rand = crypto.randomBytes(16).toString('hex');
  const sig = crypto.createHmac('sha256', DOWNLOAD_SECRET)
    .update(`${orderId}:${productId}:${rand}`)
    .digest('hex')
    .slice(0, 16);
  return `${rand}${sig}`;
}

function verifyTokenSignature(token, orderId, productId) {
  if (!token || token.length !== 48) return false;
  const rand = token.slice(0, 32);
  const givenSig = token.slice(32);
  const expected = crypto.createHmac('sha256', DOWNLOAD_SECRET)
    .update(`${orderId}:${productId}:${rand}`)
    .digest('hex')
    .slice(0, 16);
  try {
    return crypto.timingSafeEqual(Buffer.from(givenSig, 'hex'), Buffer.from(expected, 'hex'));
  } catch { return false; }
}

// Gated PDF download endpoint — looks up the token in Firestore to find
// the matching order and product, then streams the file
app.get('/dl/:token', async (req, res) => {
  const token = req.params.token;
  if (!token || !/^[a-f0-9]{48}$/.test(token)) return res.status(403).send('Invalid download link.');
  if (!db) return res.status(500).send('Service unavailable.');
  try {
    // Find the download record by token
    const snap = await db.collection('downloads').where('token', '==', token).limit(1).get();
    if (snap.empty) return res.status(403).send('Invalid or unrecognised download link. Go to My Orders to get a fresh one.');
    const dlDoc = snap.docs[0].data();
    // Verify the token was actually signed by this server
    if (!verifyTokenSignature(token, dlDoc.orderId, dlDoc.productId)) {
      return res.status(403).send('Invalid download link.');
    }
    const product = PRODUCTS[dlDoc.productId];
    if (!product || !product.pdfPath) return res.status(404).send('File not found.');
    ...
    // Log the download (useful for analytics, not blocking)
    db.collection('downloads').doc(snap.docs[0].id).update({ lastDownloadedAt: admin.firestore.Timestamp.now(), downloadCount: (dlDoc.downloadCount || 0) + 1 }).catch(() => null);
    return res.sendFile(filePath, err => {
      if (err) { console.error('Download error:', err.message); if (!res.headersSent) res.status(500).send('Could not serve file.'); }
    });
  } catch (err) {
    console.error('/dl error', err.message);
    return res.status(500).send('Could not process download request.');
  }
});
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      rows.push({
        ...
        items: items.map(it => ({
          ...it,
          // Use the stored pdfUrl (which contains the permanent token) as-is
          pdfUrl: it.pdfUrl || (it.productId ? `/dl/${mintDownloadToken(doc.id, it.productId)}` : null)
        }))
      });
    });
...
app.post('/api/verify', payLimiter, async (req, res) => {
  try {
    ...
    // Generate permanent download tokens for each item — stored in Firestore,
    // never expire, cryptographically unguessable
    const ngnAmountPaid = (tx.amount || 0) / 100;
    const usdTotal = metadata.usd_total || metadata.usd_price || 0;

    // We need the order ID before we can mint tokens (tokens include orderId in their HMAC),
    // so we generate the order doc ref first, then save everything together
    const orderRef = db ? db.collection('my_order').doc() : null;
    const orderId = orderRef ? orderRef.id : tx.reference;

    const items = productIds.map(id => {
      const product = PRODUCTS[id];
      if (!product) return null;
      const token = mintDownloadToken(orderId, product.id);
      return {
    ...
    const record = {
      ...
      items: items.map(it => ({ productId: it.productId, title: it.title, token: it.token, pdfUrl: it.pdfUrl, coverUrl: it.coverUrl })),
    ...
    if (db) {
      // Save the order
      if (orderRef) {
        await orderRef.set(record).catch(() => null);
      } else {
        await db.collection('my_order').add(record).catch(() => null);
      }
      // Save each download token to its own doc for fast lookup in /dl/:token
