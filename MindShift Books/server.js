...
// ── TEMP: client-error → server log ──────────────────────────────────────
// Added so the "this query needs a composite index" error from the new
// profile.html pagination (which normally only shows in browser devtools,
// with the create-index link inside e.message) shows up in these server
// logs instead. Safe to delete this whole block once that index is
// confirmed created — it's not meant as permanent error tracking.
app.post('/api/debug/log', (req, res) => {
  const { context, code, message } = req.body || {};
  console.error(`[client-log]${context ? ' ' + context : ''}${code ? ' code=' + code : ''}:`, String(message || '').slice(0, 2000));
  res.json({ ok: true });
});
...
// The product catalog, gated downloads, free eBooks library + reader, book
// preview pages, /config, and the schema.org endpoint all now live in
// ./server/bookstore.js as an Express Router, mounted here. PRODUCTS and
// mintDownloadToken are still needed directly below (in /api/track,
// /api/orders, /api/pay, etc. — not yet moved), so they're pulled back out
// of the same module.
const bookstoreRouter = require('./server/bookstore');
const { PRODUCTS, mintDownloadToken } = bookstoreRouter;
...
// Wishlist + Saved are now tabs on one combined My Library page.
app.get('/wishlist', (req, res) => {
  res.redirect(301, '/library?tab=wishlist');
});
...
// ── Content moderation (articles + posts) ───────────────────────────────────
// GET    /api/admin/content?type=articles|posts|all
// DELETE /api/admin/content/:type/:id   (type is 'articles' or 'posts')
//
// Articles and posts now publish straight from the client the moment the
// author hits Publish/Post (see firestore.rules -- the author's own uid has
// to match authorUid, same trust boundary as everything else client-owned).
// There's no pending/review queue anymore, so this replaces the old
// submissions-based approve/reject flow: it's a live list of everything
...
app.get('/api/admin/content', async (req, res) => {
  try {
    ...
    const collections = type === 'all' ? ['articles', 'posts'] : [type];
    if (collections.some(c => c !== 'articles' && c !== 'posts')) {
      return res.status(400).json({ error: 'type must be articles, posts, or all' });
    }
    const snaps = await Promise.all(collections.map(c =>
      db.collection(c).orderBy('createdAt', 'desc').limit(300).get()
    ));
    const items = [];
  ...
  try {
    ...
    if (s.authorUid) {
      await db.collection('notifications').add({
        recipientUid: s.authorUid,
        type: type === 'articles' ? 'article_removed' : 'post_removed',
        actorUid: 'official',
    ...
    console.error('/api/admin/content/:type/:id delete error', err);
...
app.listen(PORT, () => {
  ...
  // Pre-warm the free-eBooks cache right after boot — see
  // ./server/bookstore.js's warmupFreeEbooksCache for what this actually
  // does and why (otherwise the first visitor to any category besides
  // "All" eats a slow live Gutendex round-trip).
  bookstoreRouter.warmupFreeEbooksCache();
});
